...
RSpec.describe RailsAIPromptable::Promptable do
  ...
  describe '.prompt_template' do
    context 'when called with a template' do
      it 'sets the ai_prompt_template' do
        test_class.prompt_template('Hello %{name}!')
        expect(test_class.ai_prompt_template).to eq('Hello %{name}!')
      end
    end
  ...
  describe '#ai_generate' do
    before do
      test_class.prompt_template('Hello %{name}, you are %{age} years old.')
  ...
  describe '.ai_use_template' do
    ...
    context 'when template exists in registry' do
      it 'loads the template from registry' do
        RailsAIPromptable::TemplateRegistry.register(:greeting, 'Hello %{name}!')
    ...
    context 'when template does not exist in registry' do
      it 'raises ArgumentError with helpful message' do
        ...
        }.to raise_error(ArgumentError, /Template 'nonexistent' not found.*existing/)
      end
    end
  end
  ...
  describe '#render_template (private)' do
    it 'renders template with symbol keys' do
      test_class.prompt_template('Hello %{name}!')
      result = test_instance.send(:render_template, 'Hello %{name}!', { name: 'World' })
      expect(result).to eq('Hello World!')
    end

    it 'handles missing keys gracefully with fallback' do
      test_class.prompt_template('Hello %{name}, %{missing}!')
      result = test_instance.send(:render_template, 'Hello %{name}, %{missing}!', { name: 'World' })
      expect(result).to eq('Hello World, %{missing}!')
    end
  end
end

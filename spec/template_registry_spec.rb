...
RSpec.describe RailsAIPromptable::TemplateRegistry do
  ...
  describe ".register" do
    it "registers a template with a name" do
      described_class.register(:summary, "Summarize: %<text>s")
      expect(described_class.get(:summary)).to eq("Summarize: %<text>s")
    end
  ...
  describe ".get" do
    ...
    it "retrieves registered templates" do
      described_class.register(:greeting, "Hello %<name>s")
      expect(described_class.get(:greeting)).to eq("Hello %<name>s")
    end
  end
  ...
  describe ".load_from_file" do
    it "loads templates from a YAML file" do
      Tempfile.create(["templates", ".yml"]) do |file|
        ...
        expect(described_class.get(:summary)).to eq("Summarize: %<content>s")
        expect(described_class.get(:greeting)).to eq("Hello %<name>s")
      end
    end
  ...
  describe ".load_from_directory" do
    it "loads templates from text files in directory" do
      Dir.mktmpdir do |dir|
        File.write(File.join(dir, "summary.txt"), "Summarize: %<content>s")
        File.write(File.join(dir, "greeting.txt"), "Hello %<name>s")
    ...
    it "loads templates from YAML files in directory" do
      Dir.mktmpdir do |dir|
        File.write(File.join(dir, "summary.yml"), { "template" => "Summarize: %<content>s" }.to_yaml)

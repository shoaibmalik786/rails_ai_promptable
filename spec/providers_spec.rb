...
RSpec.describe RailsAIPromptable::Providers do
  ...
  describe '.for' do
    ...
    context 'when provider is unknown' do
      it 'raises an ArgumentError with helpful message' do
        ...
        }.to raise_error(ArgumentError, /Unknown provider: unknown.*Supported providers/)
      end
    end
  end
  ...
  describe '.available_providers' do
    it 'returns an array of available provider symbols' do
      expect(described_class.available_providers).to eq([:openai, :anthropic, :gemini, :cohere, :azure_openai, :mistral, :openrouter])
    end
  end
end
